"""BioTek Gen5 visualization — 96-well plate heatmap plots."""
...
def plot_plate_heatmap(
    ...
    cmap = "YlGn" if reading.mode == ReadingMode.FLUORESCENCE else "YlOrRd"
    ...
    for i in range(len(rows)):
        for j in range(len(cols)):
            ...
            color = "white" if val > (data_max * 0.6) else "black"
            ax.text(j, i, text, ha="center", va="center", fontsize=6, color=color)

    # Colorbar
    cbar = plt.colorbar(im, ax=ax, fraction=0.02, pad=0.04)
    unit = "RFU" if reading.mode == ReadingMode.FLUORESCENCE else "OD"
    cbar.set_label(unit)
