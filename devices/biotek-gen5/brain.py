"""Gen5Brain — AI-powered plate reader data interpretation via Claude."""
...
PLATE_READER_SYSTEM_PROMPT = (
    "You are an expert bioassay scientist working as part of an AI scientist system.\n"
    "\n"
...
The assay shows excellent dynamic range with a 15-fold signal-to-noise ratio. The serial dilution
in columns 1-2 produces the expected decreasing OD pattern from rows A through H, consistent with
a well-prepared standard curve.

No significant edge effects detected. The blank wells (cols 11-12) show uniform low background.

## Key Findings

1. **Standard curve**: Clear dose-response in columns 1-2, suitable for 4PL curve fitting
2. **Sample wells (cols 3-10)**: Show a range of OD values from 0.3 to 1.8
...
The Z-factor of 0.875 indicates an excellent assay with clear separation between positive and
negative controls. This is well above the 0.5 threshold for a high-quality screening assay.

## Key Findings

1. **Drug response gradient**: Sample wells (cols 3-10) show dose-dependent fluorescence decrease
2. **Concentration-response curve**: IC50 appears to be in the middle concentration range (cols 6-7)
...
3. **Counter-screen**: Run cytotoxicity assay (LDH release) to distinguish effects
...
class Gen5Brain(BaseBrain):
    """AI-powered plate reader interpretation using Claude.

    When ANTHROPIC_API_KEY is set, calls go to the live Claude API.
    Without the key, falls back to pre-cached demo responses so the
    skill works offline for demos and testing.

    Implements BaseBrain.analyze() and BaseBrain.summarize(), plus the
    richer interpret_reading() method for streaming plate data analysis.
    """
    ...
    def __init__(self, model: str = "claude-sonnet-4-20250514") -> None:
        ...
        if self._use_api:
            ...
            self.client = None
            logger.info("ANTHROPIC_API_KEY not set — Gen5Brain will use cached responses")
        self.model = model

    # -- BaseBrain interface -----------------------------------------------

    def analyze(self, data: dict[str, Any], context: str = "") -> str:
        """Analyze plate reader data dict and return interpretation string.

        Args:
            data: Dict with a 'reading' key containing a PlateReading, or
                  a plain dict of well values for simpler use cases.
        ...
        if reading is not None and isinstance(reading, PlateReading):
            result = self.interpret_reading(reading, context=context, stream=False)
            if not isinstance(result, str):
                msg = (
                    "Expected str from interpret_reading(stream=False), "
                    f"got {type(result).__name__}"
                )
                raise TypeError(msg)
            return result

        # Fallback: summarize the dict as plain text
        lines = [f"{k}: {v}" for k, v in data.items()]
        summary = "\n".join(lines)
        if not self._use_api:
            return f"Cached analysis unavailable for raw dict input.\n\nData:\n{summary}"

        return self._call(PLATE_READER_SYSTEM_PROMPT, f"Analyze this data:\n{summary}")

    def summarize(self, findings: list[str]) -> str:
        """Summarize a list of analysis findings into a coherent narrative.

        Args:
            findings: List of individual finding strings from analyze().
        ...
        prompt = f"Summarize these plate reader analysis findings:\n\n{combined}"

        if not self._use_api:
            # Simple offline summary
            return f"Summary of {len(findings)} finding(s):\n\n" + "\n---\n".join(findings)

        return self._call(PLATE_READER_SYSTEM_PROMPT, prompt)
    ...
    def interpret_reading(
        ...
        """Send plate reader data to Claude for full interpretation.

        Args:
            reading: PlateReading with well data and metadata.
            context: Optional additional experimental context.
            stream: If True, return a generator of text chunks.

        Returns:
            Full analysis string, or a generator of chunks if stream=True.
        """
        if not self._use_api:
    ...
    def _build_summary(self, reading: PlateReading) -> str:
        """Build a text summary of plate reader data for the LLM."""
        lines = [
    ...
    def _cached_or_error(
        self, reading: PlateReading, stream: bool
    ) -> str | Generator[str, None, None]:
        ...
        logger.info("Serving cached Gen5 response for '%s'", cache_key)
        if stream:
