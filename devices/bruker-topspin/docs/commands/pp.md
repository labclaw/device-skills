...
1. Append peaks to list: When it is checked, the found peaks are appended to a possibly existing list. When it is unchecked, a new list is created [pp2d  append]
2. Discard new peak(s) if already in list: Check this option to avoid duplicate peaks [pp2d noduplicates]
3. Export results as XwinNmr peak list: In addition to TopSpin XML format, the result is also stored in XWIN-NMR format (file peak.txt) [pp2d txt]. This file is typically used with XWIN-NMR AU programs.
...
3. Range defined by stored parameters - range stored in parameters F1P/F2P 
...
1. PPDIAG - diagonal gap; minimum distance between picked peaks and diagonal signals. Mainly used for homonuclear spectra. 
2. PPRESOL - peak picking resolution
