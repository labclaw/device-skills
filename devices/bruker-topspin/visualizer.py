...
if TYPE_CHECKING:
    from .processor import NMRSpectrum
...
def plot_spectrum(
    ...
    Args:
        spectrum: Processed NMRSpectrum to plot.
        output_path: File path to save, or None to return PNG bytes.
        title: Override plot title (defaults to sample/title from spectrum).
        annotate_peaks: Whether to label peak positions on the plot.
        ppm_range: (low, high) ppm range to display. Defaults to (-0.5, 12.0).
    ...
    if annotate_peaks and spectrum.peaks:
        ...
        for peak in spectrum.peaks:
            ...
            if rel > 0.01 and len(ppm_plot) > 0 and ppm_plot.min() <= peak.ppm <= ppm_plot.max():
